...
abstract class TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    def descendingByRetweetIter(ts: TweetSet, tl: TweetList) : TweetList = {
      if (ts.isEmpty) tl
      else {
        val head = ts.mostRetweeted
        new Cons(head, descendingByRetweetIter(ts.remove(head), tl))
      }
    }
...
class NonEmpty(val elem: Tweet, val left: TweetSet, val right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    val lur = leftAcc union rightAcc
    if (p(elem)) lur.incl(elem) else lur
  }
...
object GoogleVsApple {
  ...
  def tweetsContaining(words : List[String]) = (t:Tweet) => words.exists(x => t.text.contains(x))

  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(tweetsContaining(google))
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(tweetsContaining(apple))

  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def sentenceAnagrams(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty)
        ...
        for {
          combinationsOccurrences <- combinations(occurrences)
          word <- dictionaryByOccurrences(combinationsOccurrences)
          rest <- sentenceAnagrams(subtract(occurrences, combinationsOccurrences))
        } yield word :: rest
      }
    }

...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    if (points.isEmpty) means.map(mean => (mean, GenSeq[Point]())).toMap
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    val zip = oldMeans.zip(newMeans)
    zip.forall { case (oldMean, newMean) => oldMean.squareDistance(newMean) <= eta }
  }

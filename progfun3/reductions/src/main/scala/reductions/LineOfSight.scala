...
object LineOfSight {
  ...
  def upsweep(input: Array[Float], from: Int, end: Int, threshold: Int): Tree = {
    ...
    if (chunk > threshold) {
      val half = chunk / 2
      val (left, right) = common.parallel(upsweep(input, from, from + half, threshold), upsweep(input, from + half, end, threshold))
      Node(left, right)
    } else {
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float, tree: Tree): Unit = {
    tree match {
      case Node(left, right) => common.parallel(downsweep(input, output, startingAngle, left), downsweep(input, output, max(startingAngle, left.maxPrevious), right))
  ...
  def parLineOfSight(input: Array[Float], output: Array[Float], threshold: Int): Unit = {
    downsweep(input, output, startingAngle = 0, tree = upsweep(input, from = 1, end = input.length, threshold))
  }
}
